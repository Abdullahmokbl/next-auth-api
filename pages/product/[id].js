...
export default function Product({ product, id, user }) {
  ...
  const AddButton = () => {
    return (
      <div
        className={`button ${disabled ? "disable" : undefined}`}
  ...
  const Quantity = () => {
    return (
      <div className={styles.qty}>
        <div
          className={
            exist?.qty <= 1 || exist?.qty === undefined ? "disable" : undefined
          }
        ...
        <span>
          {disabled ? <FontAwesomeIcon icon={faSpinner} spin /> : exist.qty}
        </span>
  ...
  return (
    <div className={`${styles.product} container navpd`}>
      {product ? (
        ...
        <div className={styles.error}>This item doesn't exist</div>
      )}
    </div>
  );
}
